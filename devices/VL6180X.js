...
VL6180X.prototype.readRange = function() {
  while (! (read8(C.VL6180X_REG_RESULT_RANGE_STATUS) & 0x01));                // wait for device to be ready for range measurement

  this.write8(C.VL6180X_REG_SYSRANGE_START, 0x01);                            // Start a range measurement

  while (!(read8(C.VL6180X_REG_RESULT_INTERRUPT_STATUS_GPIO) & 0x04));        // Poll until bit 2 is set

  var range = read8(C.VL6180X_REG_RESULT_RANGE_VAL);                          // read range in mm
